...
def calzar_por_factura(FBL3N, FBL5N):
    ...
    # Obtener columnas
    fecha_valor, n_fact, fecha_cont = [FBL3N.columns[i] for i in (9, 19, 20)]
    referencia = FBL5N.columns[6]
    
    # Filtrar por fecha
    if len(FBL3N) > 0:
        FBL3N = FBL3N[
            pd.to_datetime(FBL3N[fecha_valor], "%d.%m.%Y")
            <= datetime.strptime(FBL3N[fecha_cont].iloc[0], "%d.%m.%Y")
        ]
    
    FBL3N[n_fact] = FBL3N[n_fact].astype(str)
    FBL5N[referencia] = FBL5N[referencia].astype(str)

    df = pd.merge(FBL3N, FBL5N, left_on=n_fact, right_on=referencia)
...
def extraer_datos(FBL3N, FBL5N):
    ...
    # Obtener columnas
    fecha_valor, monto_3, cliente, fecha_cont = [
        FBL3N.columns[i] for i in (9, 11, 20, 27)
    ]

    cuenta, demora, monto_5 = [
        FBL5N.columns[i] for i in (2, 12, 14)
    ]
...
def calzar_por_factura(FBL3N, FBL5N):
    ...
    return df
>>>>>>> 9d9a7c13d916eecb05a8a9377c0d96a4d57e8b5e:matches_2000.py
