...
def main():
    ...
    if opcion != "3":
        ...
        descripcion_bci = pd.read_excel(
            f'{input_dir}/Descripcion BCI.xlsx').drop(0)  # Se asume que SIEMPRE va a tener el mismo nombre
        descripcion_bci = bci(dimerc_bancos, descripcion_bci)
        print('Listo descripción BCI')
    ...
    # Validar que la fecha esté en el formato indicado y no sea una fecha futura
        ...
        while not flag_1 or not flag_2:
            ...
            if not flag_1:
                fecha_cont = input(
                    "La fecha debe ser en el formato (DD.MM.AAAA): ")
            else:
                try:
                    flag_2 = (datetime.strptime(
                        fecha_cont, "%d.%m.%Y").date() <= hoy)
                    if not flag_2:
                        fecha_cont = input(
                            "Debe ingresar una fecha no futura (DD.MM.AAAA): ")
                except:
                    flag_2 = False
                    fecha_cont = input(
    ...
    # Pedir información adicional para dar formato a datos
        ...
        while len(string) > 16:
            string = input(
                "Debe ingresar un string de no más de 16 caracteres de largo: ")
        datos["XBLNR"] = string

        string = input("Ingrese BKTXT (texto). Ej: 202201_001: ")
        while len(string) > 10:
            string = input(
                "Debe ingresar un string de no más de 10 caracteres de largo: ")
        datos["BKTXT"] = string

        string = input("Ingrese AUGTX (texto). Ej: BancoSantander: ")
        while len(string) > 18:
            string = input(
                "Debe ingresar un string de no más de 18 caracteres de largo: ")
        datos["AUGTX"] = string
    ...
    # Dar formato y escribir archivos
        ...
        if opcion in ("2", "3"):
            ...
            MATCHES_FACT_EXPORTAR = formatear_para_exportar_fact(
                MATCHES_FACT, datos)
            del MATCHES_FACT
